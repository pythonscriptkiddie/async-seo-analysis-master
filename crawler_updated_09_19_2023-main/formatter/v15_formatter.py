...
from .core.slug import make_slug, categories_from_url
...
process_categories = partial(process_func, new_field='categories',
    existing_field='url', func=categories_from_url)
...
async def make_formatted_page(item: Dict) -> Dict:
    ...
    new_item['categories']: Optional[List] = item.get('categories', '')
    new_item['keywords'] = item.get('soup_bigrams_trigrams').get('keywords', [])
    new_item['bigrams'] = Counter(item.get('soup_bigrams_trigrams').get('bigrams', None))
    new_item['trigrams'] = Counter(item.get('soup_bigrams_trigrams').get('trigrams', None))
...
async def main(pages: List[Dict]):
    ...
    step4 = await new_formatter.process_item_threads(
        items=step3, func=process_categories)
    step5 = await new_formatter.process_item_processes(
        items=step4, func=process_bigrams_trigrams_headings)
    step6 = await new_formatter.apply_to_iterable(items=step5, func=make_formatted_page)
    return step6

async def format_pages(pages: List[Dict]) -> Dict:
    '''A thin wrapper around the main() function allowing it to be
    called from outside the module.'''
    search_result : Dict = {}
    search_result['pages'] = await main(pages)
    bigram_counters = (page.get('bigrams') for page in search_result.get('pages'))
    search_result['bigrams'] = reduce(operator.add, bigram_counters)
    trigram_counters = (page.get('trigrams') for page in search_result.get('pages'))
    search_result['trigrams'] = reduce(operator.add, trigram_counters)
