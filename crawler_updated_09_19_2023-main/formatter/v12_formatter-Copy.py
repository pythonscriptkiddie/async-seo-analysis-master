...
async def main(pages: List[Dict]):
    ...
    step3 = await new_formatter.process_item_processes(
        items=step2, func=process_bigrams_trigrams_headings)
    step4 = await new_formatter.apply_to_iterable(items=step3, func=make_formatted_page)
    return step4
